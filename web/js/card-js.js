 ...
 */
CardJs.prototype.refreshCVCValidation = function() {
    var cvc = this.getCvc();
    var numbersOnly = CardJs.numbersOnlyString($(this.cvcInput).val());
    this.cvcInput.removeClass('has-error');
    if (numbersOnly.length == 0) {
        this.cvcInput.addClass('has-error');
    }
};
 ...
 *
 */
CardJs.prototype.refreshCreditCardValidation = function() {

    this.cardNumberInput.removeClass('has-error');
    var number = CardJs.numbersOnlyString(this.cardNumberInput.val());

    if (!CardJs.isValidLuhn(number)) {
        this.cardNumberInput.addClass('has-error');
        return;
    }

    if (!CardJs.isValidLength(number)) this.cardNumberInput.addClass('has-error');
};

/**
 *
 */
CardJs.prototype.refreshCreditCardLuhnValidation = function() {
    var isValid = CardJs.isValidLuhn(CardJs.numbersOnlyString(this.cardNumberInput.val()));
    this.cardNumberInput.removeClass('has-error');
    if (!isValid) this.cardNumberInput.addClass('has-error');
};
...
CardJs.isValidLength = function(number) {
    var typeValid = false;
    var type = CardJs.cardTypeFromNumber(number);
    switch(type) {
      case "Visa Electron":
      case "Visa":
        typeValid = true;
...
CardJs.isValidLuhn = function(number) {
...
CardJs.prototype.initCardNumberInput = function() {
  ...
  this.cardNumberInput.keyup(function() {
    $this.refreshCreditCardTypeIcon();
    $this.refreshCreditCardLuhnValidation();
  });
  ...
  this.cardNumberInput.on('paste', function() {
    setTimeout(function() {
      ...
      $this.refreshCreditCardTypeIcon();
      $this.refreshCreditCardValidation();
    }, 1);
  });
...
/**
 * Is the given card number valid?
 *
 * @param number
 * @returns {boolean}
 */
CardJs.isNumberValid = function(number) {
    if (!CardJs.isValidLuhn(number)) {
        return false;
    }

    return CardJs.isValidLength(number);
};
(function($) {
  ...
  var methods = {
    ...
    validateInputs: function() {
        var cardNumber = CardJs.numbersOnlyString(this.data("cardjs").getCardNumber());
        var expiryMonth = this.data("cardjs").getExpiryMonth();
        var expiryYear = this.data("cardjs").getExpiryYear();
        var cvc = CardJs.numbersOnlyString(this.data("cardjs").getCvc());

        if (!CardJs.isNumberValid(cardNumber)) {
            this.data("cardjs").refreshCreditCardValidation();
            return false;
        }

        if (!CardJs.isExpiryValid(expiryMonth, expiryYear)) {
            this.data("cardjs").refreshExpiryMonthValidation()
            return false;
        }

        if (cvc.length == 0) {
            this.data("cardjs").refreshCVCValidation();
            return false;
        }
        return true;
    }
